...
function initWorkspace(featureId){
    ...
    if(f.isRoleplay){
        ...
        <div class="ws-panel" style="display:flex;flex-direction:column;">
            ...
            <div class="wx-input-bar">
                ${f.allowVoice?'<button class="wx-voice-btn" id="wxVoiceBtn" data-target="roleplayInput" onclick="toggleVoiceRecording(this)">🎙️</button>':''}
    ...
    // 绑定上传按钮事件（修复手机端上传按钮不可用的问题）
    if(showUpload) {
        const wxPlusBtn = document.getElementById('wxPlusBtn');
        const fileInput = document.getElementById('fileInput');
        if(wxPlusBtn && fileInput) {
            wxPlusBtn.addEventListener('click', () => fileInput.click());
        }
    }
}
